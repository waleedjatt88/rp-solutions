  ...
  const coreValues = [
    {
      ...
      // CHANGE 1: Icon size ko thora chota kiya
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12">
          {coreValues.map((value) => (
            <div key={value.title} className={`p-1.5 rounded-t-[60px] rounded-b-[20px] ${themeColor.bgGradient} shadow-2xl`}>
                {/* CHANGE 2: Card ki padding kam ki (p-8 se p-6) */}
                <div className="relative bg-white rounded-t-[60px] rounded-b-[12px] h-full p-6 text-center flex flex-col items-center">
                    ...
                    {/* --- Shiny Icon Container --- */}
                    {/* CHANGE 3: Icon container ka size aur negative margin kam kiya */}
                    ...
                    {/* Title */}
                    {/* CHANGE 4: Title ka top margin kam kiya (mt-6 se mt-4) */}
                    ...
                    {/* Step Number div ko hata diya gaya hai taake faltu space na le */}
                </div>
            </div>
          ))}
        </div>
